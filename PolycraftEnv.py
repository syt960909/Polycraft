...
class PolycraftHGEnv:
    ...
    def reset(self, domain_file):
        ...
        #command = 'restart'
        #print(domain_file)
        #command = "RESET domain " + 'C:\Users\ysun465\Downloads\Polycraft-test\experiments\HUGA_L00_T01_S01_VIRGIN_X1000_U9999_V1_G00000_I0758_N0.json'
        ...
        screen = Image.open(io.BytesIO(screen))
        screen = np.asarray(screen)

        return screen
        
    def step(self, action):
        #rad_act = choice(self.l)
        #print(rad_act)
        #rad_func = self.aug_to_func[rad_act]
        ...
        screen = Image.open(io.BytesIO(screen))
        screen = np.asarray(screen)
        #screen = rad_func(screen)
        reward = self.get_reward(state_dict, act_dict)
        #print(reward)
        done = self.goal0_achieved(state_dict)
        done_ = self.goal1_achieved(state_dict)
        info = {}
        ...
        return screen, reward, done,done_, info
    ...
    def goal0_achieved(self, state_dict):
        if self.opt.mode == 'all':
            ...
            #if self.have_macguffin(state_dict):
                #goal_ach = (player_pos==dest_pos&self.have_macguffin(state_dict)==True)
             #   goal_ach = True
            goal0_ach = self.have_macguffin(state_dict)

        return goal0_ach

    def goal1_achieved(self, state_dict):
        if self.opt.mode == 'all':
            goal_ach = self.have_macguffin(state_dict)
            """
            goal_ach = False
    ...
    def get_reward(self, state_dict, act_dict):
        ...
        if self.opt.mode == 'all':
            ...
            if act_dict['command_result']['result'] == 'FAIL':
                reward -= 0.5
            if self.have_macguffin(state_dict):
                #print(self.have_macguffin(state_dict))
                ...
                if self.get_macguffin:
                    reward -= 150.0
                    self.get_macguffin = False
            if self.goal1_achieved(state_dict):
                reward += 2000.0
        ########################################################################
        else:
            # move: -0.3
